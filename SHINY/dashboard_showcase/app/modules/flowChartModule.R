      ...
      column(3,
             div(id = ns("Selector1"),
                 selectizeInput(inputId  = ns("SankeyStreams"),"input streams",
                                choices  = c("Oncology","CV","MS","PAH"),
                                selected = c("PAH","MS","Oncology"),
                                multiple = TRUE,
...
flowChartModule <- function(input, output, session, moduleData,helpReactive,
  ...
  # helper function to get Sankey chart hours
  GetSankeyHours <- function(l1,l2){
  ...
  chartData <- reactive({
    ...
    # for each pair compute hours, store pairs in data
    data <- NULL
    ...
    for(i in 1:NrOfStreamPairs){
      ...
      # add prefix to prevent cycles e.g. when MS and label have an overlap in names
      m      <- match(name1,c("Oncology","CV","MS","PAH"))
      prefix <- c("onc","lab","prod","PAH")[m]
      ...
      m      <- match(name2,c("Oncology","CV","MS","PAH"))
      prefix <- c("proc","lab","prod","PAH")[m]
      v2     <- paste(prefix,v2,sep="_")

      Hours <- GetSankeyHours(v1,v2)

      # update data
      data   <- rbind(data,Hours)
    }
  ...
  chartData2 <- reactive({
    ...
    data <- GetSankeyHours(v1,v2)

    # link to structure
    v3   <- lstData$DD.LogIndicator[m,input$SankeyStreams3]

    # add prefix to prevent cycles e.g. when MS and label have an overlap in names
    m      <- match(input$SankeyStreams3,c("Oncology","CV","MS","PAH"))
    prefix <- c("proc","lab","prod","PAH")[m]
    v3     <- paste(prefix,v3,sep="_")

    # get hours
    data2 <- GetSankeyHours(v2,v3)

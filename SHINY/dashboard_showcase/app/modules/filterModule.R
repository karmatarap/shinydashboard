...
filterModuleUI <- function(id){
  ...
  tagList(
    ...
    fluidRow(
      column(3,uiOutput(ns("FilterOncology"))),
      column(3,uiOutput(ns("FilterCV"))),
      column(3,uiOutput(ns("FilterMS"))),
...
### Build ui components ----
  output$FilterOncology <- renderUI({

    items          <- RV$items$Oncology
    selectedItems  <- RV$selected.items$Oncology
    implied.items  <- RV$implied.items$Oncology

    buttonGroup(ns("Oncology"),items,selectedItems = selectedItems,label="Oncology",implied.items = implied.items)
  })

  output$FilterCV <- renderUI({

    items          <- RV$items$CV
    selectedItems  <- RV$selected.items$CV
    implied.items  <- RV$implied.items$CV

    buttonGroup(ns("CV"),items,selectedItems = selectedItems,label="CV",implied.items = implied.items)
  })

  output$FilterMS <- renderUI({

    items          <- RV$items$MS
    selectedItems  <- RV$selected.items$MS
    implied.items  <- RV$implied.items$MS

    buttonGroup(ns("MS"),items,selectedItems = selectedItems,label="MS",implied.items = implied.items)
  })

  output$FilterPAH <- renderUI({

    items          <- RV$items$PAH
    selectedItems  <- RV$selected.items$PAH
    implied.items  <- RV$implied.items$PAH

    buttonGroup(ns("PAH"),items,selectedItems = selectedItems,label="PAH",implied.items = implied.items)
  })
...
### Start reset controls ----
  ...
  observeEvent(RV$reset,{
    resetButtonGroup(session, 'Oncology')

    resetButtonGroup(session, 'CV')

    resetButtonGroup(session, 'MS')

    resetButtonGroup(session, 'PAH')
...
### Start Deleting\Saving\Loading filter----
  ...
  observeEvent(input$load,{
    ...
    if(input$filterName=="None"){
      ...
      RV$selected.items      <- list(Oncology           = currentFilter$Signaleringen.Filter$Oncology$selected ,
                                     CV            = currentFilter$Signaleringen.Filter$CV$selected  ,
                                     MS          = currentFilter$Signaleringen.Filter$MS$selected,
                                     PAH          = currentFilter$Signaleringen.Filter$PAH$selected,
...
### End Deleting\Saving\Loading filter----
  ...
  ###
  observe({
      RV$Signaleringen.Filter   <- list(Oncology=input$Oncology,label=input$CV,MS=input$MS,PAH=input$PAH,

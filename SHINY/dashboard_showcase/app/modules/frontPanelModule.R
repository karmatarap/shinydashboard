...
frontPanelModule <- function(input, output, session,
                             ...
                             DefaultColors=list('RED','GREEN','BLACK','ORANGE')){
...
# initialization ----
  ...
  dynamicValues <- reactive({
    ...
    lstReturn['%GAUGECOLOR%']      <- c("blue")#input$ResultType
    print(input$ResultType)
...
# start Pies modules ----
  piesData <- reactive({
    list(OncologyHours=GetC3PieHours(brushedData(),'Oncology',5),
                              CVHours=GetC3PieHours(brushedData(),'CV',5),
                              PAHHours=GetC3PieHours(brushedData(),'PAH',5),
                              MSHours=GetC3PieHours(brushedData(),'MS',5))})
...
### Reactives ---
  ...
  chartTimeData <- reactive({
    ...
    retChartTimeData <- createChartHours(lstData$DD.Signaleringen,'eindargumentatie',FALSE)
  ...
  lineChartData <- reactive({
    ...
    Hours <- chartTimeData()[,-1]

    if(class(Hours)=='numeric'){
      Hours = as.data.frame(Hours)
      names(Hours) = names(chartTimeData())[-1]
    }

    Total  <- apply(Hours,1,sum,na.rm = TRUE)
    Hours <- round(100*Hours / Total, 3)
    retData   <- data.frame(Time = chartTimeData()[1], Hours, Total)
...
#outputs ----
  ...
  output$lineBarChart <- renderFrissC3LineBarChart({
    ...
    FrissC3LineBarChart(as.character(lineChartData()$Time),
                        ...
                        lineData=lineData,
                        color=c('gray',colors),
                        showSubChart=TRUE,debounce=200,height=300,yLabel1="Total hours",yLabel2="Percentage",xLabel="Week")
  })
  ...
  output$SelectResult <- renderUI({
    ...
    selectizeInput(ns("ResultType"),"Deliverable:",choices = Choices, selected = selectedColor, width = "100px")
  })
...
constructGaugeData <- function(ResultType,DD.Signaleringen,period=NA){
  ...
  gaugeStatColors <- list("Submission","CSR","Other","DSUR/IDMC")

  return(list(values=values,value=value,color=color,gaugeStatColors=gaugeStatColors))
}

# helper function Pie charts
GetC3PieHours <- function(DD.Signaleringen,field,maxGroups){

  hours <- DD.Signaleringen %>% group_by_(field) %>% summarise(count =n()) %>% arrange(desc(count))

  maxGroups <- min(nrow(hours),maxGroups)

  hours <- hours[1:maxGroups,]

  rest   <- nrow(DD.Signaleringen) - sum(hours$count)

  if(rest>0){
    hours <- rbind(hours,c("other",rest))
    hours <- as.data.frame(hours)
  }

  names(hours) <- c("name","count")

  PieData           <- data.frame(t(hours$count))
  colnames(PieData) <- hours$name
  return(PieData)
}
...
getColors <- function(lineData){
  ...
  colors[tolower(colors) == 'other' ]   <- 'orange'
  colors[tolower(colors) == 'submission' ]   <- 'red'
  colors[tolower(colors) == 'csr' ]   <- 'green'
  colors[tolower(colors) == 'dsur/idmc'] <- 'black'
...
#' Creates aggregated hours per week of the different end results
#'
...
#' @returns Dataframe with aggregated hours. Each row represents a week and each column an end result.
createChartHours <- function(DD.Signaleringen,colorField,useJsTime=FALSE){
  ...
  if(class(Hours)=='numeric'){
    Hours = as.data.frame(Hours)
    names(Hours) = names(DD)[-c(1:2)]
  }

  ### Make sure colors are ordered according to friss ranking
  isolate({
    ChartTimeData <- data.frame(Time = Time, Hours)

...
ShowColorInfo <- function(values){
  ...
  displayColors[displayColors=="Other"] <- "ORANGE"
  displayColors[displayColors=="Submission"] <- "RED"
  displayColors[displayColors=="CSR"] <- "GREEN"
  displayColors[displayColors=="DSUR/IDMC"] <- "BLACK"
  values <- values$pct
  nColor <- length(colors)

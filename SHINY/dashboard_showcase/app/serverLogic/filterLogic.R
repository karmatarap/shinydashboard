...
buildFilter <- reactive({
  ...
  isolate({
    items           <- list(Oncology     = unique(MappedData$DD.Signaleringen$Oncology),
                            CV      = unique(MappedData$DD.Signaleringen$CV),
                            MS    = unique(MappedData$DD.Signaleringen$MS),
                            PAH    = unique(MappedData$DD.Signaleringen$PAH),
    ...
    if(is.null(RV$signaleringen.Filter)){
      ...
      selected.items  <- list(Oncology           = RV$signaleringen.Filter$Oncology$selected ,
                              CV            = RV$signaleringen.Filter$CV$selected  ,
                              MS          = RV$signaleringen.Filter$MS$selected,
                              PAH          = RV$signaleringen.Filter$PAH$selected,
...
#'                             impl.Oncology           : list with procceses that will be implicitly filtered out
#'                             impl.CV            : list with CVs that will be implicitly filtered out
#'                             impl.MS          : list with MSs that will be implicitly filtered out
#'                             impl.PAH          : list with PAHs that will be implicitly filtered out
...
applySignaleringenFilter <- function(DD.Signaleringen,DD.LogIndicator,Signaleringen.Filter){
  ...
  F1     <- !is.element(DD.Signaleringen$Oncology,Signaleringen.Filter$Oncology$notSelected)
  F2     <- !is.element(DD.Signaleringen$CV,Signaleringen.Filter$CV$notSelected)
  F3     <- !is.element(DD.Signaleringen$MS,Signaleringen.Filter$MS$notSelected)
  F4     <- !is.element(DD.Signaleringen$PAH,Signaleringen.Filter$PAH$notSelected)
...
#
  ...
  impl.Oncology            <- setdiff(unique(DD.Signaleringen$Oncology [F1])           ,unique(DD.Signaleringen$Oncology           [     F2 & F3 & F4 & F5 & F6 & F7 & F8 & F9 & F10 & F11]))
  impl.CV             <- setdiff(unique(DD.Signaleringen$CV  [F2])           ,unique(DD.Signaleringen$CV            [F1      & F3 & F4 & F5 & F6 & F7 & F8 & F9 & F10 & F11]))
  impl.MS           <- setdiff(unique(DD.Signaleringen$MS[F3])           ,unique(DD.Signaleringen$MS          [F1 & F2      & F4 & F5 & F6 & F7 & F8 & F9 & F10 & F11]))
  impl.PAH           <- setdiff(unique(DD.Signaleringen$PAH[F4])           ,unique(DD.Signaleringen$PAH          [F1 & F2 & F3      & F5 & F6 & F7 & F8 & F9 & F10 & F11]))
  ...
  implied.items <- list(Oncology=impl.Oncology,CV=impl.CV,MS=impl.MS,PAH=impl.PAH,
